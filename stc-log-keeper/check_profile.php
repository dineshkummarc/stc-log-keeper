...
//START FULL RESULT LIST//    //START FULL RESULT LIST//    //START FULL RESULT LIST//
echo
...
if($mod_val>0){
            ...
            $ref_log_id=$il_row['attend_id'];
                ...
                <a id='logout_btn".$chk_lrn_row['member_id']."' onclick='show_logout_panel".$chk_lrn_row['member_id']."()' class='btn btn-warning' style='font-size: small; padding: 0px 2px 0px 2px; color: #ffffff; width: 100%;'><i class='fa fa-sign-out'></i> Logout</a>";
                ...
                function show_logout_panel<?php echo $chk_lrn_row['member_id']; ?>(){
                    
                    document.getElementById('logout_panel<?php echo $chk_lrn_row['member_id']; ?>').style.display = 'block';
                    document.getElementById('search_result_list_panel').style.display = 'none';
                ...
                <a id='login_btn".$chk_lrn_row['member_id']."' onclick='show_hdf_login_panel".$chk_lrn_row['member_id']."()' class='btn btn-primary' style='font-size: small; padding: 0px 2px 0px 2px; color: #ffffff; width: 100%;'><i class='fa fa-sign-in'></i> Login</a>";
                ...
                function show_hdf_login_panel<?php echo $chk_lrn_row['member_id']; ?>(){
                    
                    document.getElementById('hdf_login_panel<?php echo $chk_lrn_row['member_id']; ?>').style.display = 'block';
                    document.getElementById('search_result_list_panel').style.display = 'none';
...
//START SELECTED RESULT//   //START SELECTED RESULT//   //START SELECTED RESULT//

$chk_lrn_selected_query = $conn->query("SELECT * FROM members WHERE profile_id = '$_POST[profile_id]' OR (lname LIKE '%$_POST[profile_id]%') ORDER BY lname, fname ASC") or die(mysql_error());
    
    
echo
"<div id='selected_result_panel' style='width: 100%;'>";
            
while($chk_lrn_selected_row = $chk_lrn_selected_query->fetch()){
        
        $chk_log_selected_query = $conn->prepare('SELECT * FROM tbl_attendance WHERE member_id = :member_id AND date_mm = :date_mm AND date_dd = :date_dd AND date_yyyy = :date_yyyy');
        $chk_log_selected_query->execute(['member_id' => $chk_lrn_selected_row['member_id'], 'date_mm' => $date_mm, 'date_dd' => $date_dd, 'date_yyyy' => $date_yyyy]);
        
        $mod_select_val=$chk_log_selected_query->rowCount() % 2;
...
if($mod_select_val>0){
            ...
            //LOGOUT PANEL//
            echo
            "<div id='logout_panel".$chk_lrn_selected_row['member_id']."' style='display: none;'>
            
            <div class='row' style='border: solid 1px #33b35a; border-radius: 0.25rem; margin: 12px 1px 1px 1px; padding: 4px;'>";
            
            $in_log_query = $conn->query("SELECT attend_id, log_time FROM tbl_attendance WHERE member_id='$chk_lrn_selected_row[member_id]' AND date_mm='$date_mm' AND date_dd='$date_dd' AND date_yyyy='$date_yyyy' AND action='IN' ORDER BY attend_id DESC") or die(mysql_error());
            $il_row=$in_log_query->fetch();
            ...
            $ref_log_id=$il_row['attend_id'];
                ...
                echo
                "<div class='col-5' style='display: flex; justify-content: center; align-items: center;'>
                ".$chk_lrn_selected_row['lname'].", ".$chk_lrn_selected_row['fname']."
                </div>
                ...
                <a id='cancel_btn".$chk_lrn_selected_row['member_id']."' onclick='hide_logout_panel".$chk_lrn_selected_row['member_id']."()' class='btn btn-secondary' style='font-size: small; padding: 0px 2px 0px 2px; color: #ffffff; width: 100%;'><i class='fa fa-times'></i> Cancel</a>";
                ...
                function hide_logout_panel<?php echo $chk_lrn_selected_row['member_id']; ?>(){
                    
                    document.getElementById('logout_panel<?php echo $chk_lrn_selected_row['member_id']; ?>').style.display = 'none';
                    document.getElementById('search_result_list_panel').style.display = 'inherit';
                ...
                //LOGOUT ONLY//
                echo
                "<div class='col-12' style='display: flex; justify-content: center; align-items: center; margin-top: 12px;'>
                
                <form method='POST' action='save_log.php' style='width: 100%;'>
               
                <input name='member_id' value='".$chk_lrn_selected_row['member_id']."' type='hidden' />
                <input name='ref_log_id' value='".$ref_log_id."' type='hidden' />
                
                <div class='input-group' style='margin-bottom: 12px; width:100%;'>
             
                    <input value='WANT TO LOGOUT ".$chk_lrn_selected_row['lname'].", ".$chk_lrn_selected_row['fname']."?' class='form-control' readonly='' />
...
//LOGIN PANEL
                ...
                $chk_hdf_selected_query = $conn->prepare('SELECT * FROM tbl_daily_hdf WHERE member_id = :member_id AND date_mm = :date_mm AND date_dd = :date_dd AND date_yyyy = :date_yyyy');
                $chk_hdf_selected_query->execute(['member_id' => $chk_lrn_selected_row['member_id'], 'date_mm' => $date_mm, 'date_dd' => $date_dd, 'date_yyyy' => $date_yyyy]);
                
                if($chk_hdf_selected_query->rowCount()>0){
                
                //LOGIN PANEL//
                echo
                "<div id='login_panel".$chk_lrn_selected_row['member_id']."' style='display: none;'>";
                ...
                <a id='cancel_btn".$chk_lrn_selected_row['member_id']."' onclick='hide_login_panel".$chk_lrn_selected_row['member_id']."()' class='btn btn-secondary' style='font-size: small; padding: 0px 2px 0px 2px; color: #ffffff; width: 100%;'><i class='fa fa-times'></i> Cancel</a>";
                ...
                function hide_login_panel<?php echo $chk_lrn_selected_row['member_id']; ?>(){
                    
                    document.getElementById('login_panel<?php echo $chk_lrn_selected_row['member_id']; ?>').style.display = 'none';
                    document.getElementById('search_result_list_panel').style.display = 'inherit';
                ...
                <p class='text-info' style='margin-top: 8px; margin-bottom: 0px;'>WANT TO LOGIN ".$chk_lrn_selected_row['lname'].", ".$chk_lrn_selected_row['fname']."?</p>
               ...
               //END LOGIN PANEL//
                ...
                //HDF LOGIN PANEL//
                echo
                "<div id='hdf_login_panel".$chk_lrn_selected_row['member_id']."' style='display: none;'>";
                ...
                <a id='cancel_btn".$chk_lrn_selected_row['member_id']."' onclick='hide_hdf_login_panel".$chk_lrn_selected_row['member_id']."()' class='btn btn-secondary' style='font-size: small; padding: 0px 2px 0px 2px; color: #ffffff; width: 100%;'><i class='fa fa-times'></i> Cancel</a>";
                ...
                function hide_hdf_login_panel<?php echo $chk_lrn_selected_row['member_id']; ?>(){
                    
                    document.getElementById('hdf_login_panel<?php echo $chk_lrn_selected_row['member_id']; ?>').style.display = 'none';
                    document.getElementById('search_result_list_panel').style.display = 'inherit';
...
//END SELECTED RESULT// //END SELECTED RESULT// //END SELECTED RESULT//
